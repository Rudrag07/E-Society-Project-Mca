...
const Residence = () => {
  ...
  const isFormValid = () => {
    ...
    const config = wingConfig[formData.wing];
    const flatValid = formData.flatNumber >= config.min && formData.flatNumber <= config.max && !isAlreadyBooked;
    ...
    if (formData.method === 'QR Code') {
      ...
      paymentValid = formData.cardNumber.replace(/\s/g, '').length === 16 && expiryRegex.test(formData.expiry) && formData.cvv.length === 3;
  ...
  // --- CORE REGISTRY LOGIC (LIVE DATA SYNC) ---
  const executeRegistry = (e) => {
    e.preventDefault();
    if (isAlreadyBooked) return;

    setIsVerifying(true);
    const toastId = toast.loading("Finalizing Global Registry...");
    ...
    setTimeout(() => {
      ...
      // 1. SAVE TO RESIDENT DATABASE (For Visitor Search in Services.jsx)
      const existingResidents = JSON.parse(localStorage.getItem('society_residents')) || [];
      const updatedResidents = [...existingResidents, {
        name: formData.fullName.toUpperCase(),
        wing: formData.wing,
        flat: formData.flatNumber.toString(),
        type: formData.unitType
      }];
      localStorage.setItem('society_residents', JSON.stringify(updatedResidents));
      
      // 2. SAVE BOOKED FLATS (For UI Counters)
      ...
      // 3. SAVE LAST BOOKING (For Receipt Generation)
        ...
        <form onSubmit={executeRegistry} className="space-y-6">
          ...
          <div className="bg-[#0a0c14] p-8 rounded-[2.5rem] border border-white/10 space-y-8">
            <div className="flex bg-black p-2 rounded-2xl border border-white/5">
              <button type="button" onClick={() => setFormData({...formData, paymentType: 'Token', method: 'QR Code'})} className={`flex-1 py-4 rounded-xl text-[12px] font-bold uppercase transition-all ${formData.paymentType === 'Token' ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/20' : 'text-slate-600'}`}>Token Pay (50,000)</button>
              <button type="button" onClick={() => setFormData({...formData, paymentType: 'Full', method: 'Card'})} className={`flex-1 py-4 rounded-xl text-[12px] font-bold uppercase transition-all ${formData.paymentType === 'Full' ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/20' : 'text-slate-600'}`}>Full Payment</button>
            </div>

            <div className="grid grid-cols-3 gap-4">
              {formData.paymentType === 'Token' && (
                <button type="button" onClick={() => setFormData({...formData, method: 'QR Code'})} className={`py-6 rounded-2xl border flex flex-col items-center gap-2 transition-all ${formData.method === 'QR Code' ? 'border-emerald-500 bg-emerald-500/5 text-emerald-500' : 'border-white/5 text-slate-700'}`}><QrCode size={25}/><span className="text-[13px] font-bold">QR</span></button>
              )}
              <button type="button" onClick={() => setFormData({...formData, method: 'Card'})} className={`py-6 rounded-2xl border flex flex-col items-center gap-2 transition-all ${formData.method === 'Card' ? 'border-emerald-500 bg-emerald-500/5 text-emerald-500' : 'border-white/5 text-slate-700'}`}><CreditCard size={25}/><span className="text-[13px] font-bold">CARD</span></button>
              <button type="button" onClick={() => setFormData({...formData, method: 'Net Bank'})} className={`py-6 rounded-2xl border flex flex-col items-center gap-2 transition-all ${formData.method === 'Net Bank' ? 'border-emerald-500 bg-emerald-500/5 text-emerald-500' : 'border-white/5 text-slate-700'}`}><Landmark size={25}/><span className="text-[13px] font-bold">BANK</span></button>
            </div>
            ...
            <AnimatePresence mode="wait">
              {formData.method === 'QR Code' && (
                <motion.div key="qr" initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} exit={{opacity:0}} className="flex flex-col md:flex-row items-center gap-8 bg-black/40 p-6 rounded-3xl border border-white/5">
                  <div className="bg-white p-3 rounded-2xl shadow-2xl"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=upi://pay?pa=8200792488@axl&am=50000`} className="w-24 h-24" alt="QR" /></div>
                  <div className="w-full space-y-2">
                    <label className="text-[11px] font-bold text-slate-500 uppercase ml-1">Enter UPI ID</label>
                    <input required placeholder="username@bankid" className="w-full bg-black border border-white/10 p-5 rounded-xl text-sm font-bold lowercase outline-none" value={formData.userUpiId} onChange={e => setFormData({...formData, userUpiId: e.target.value.toLowerCase().trim()})} />
                  </div>
                </motion.div>
              )}
              {formData.method === 'Card' && (
                <motion.div key="card" initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} exit={{opacity:0}} className="space-y-4 bg-black/40 p-6 rounded-3xl border border-white/5">
                  ...
                  <input required maxLength="19" placeholder="XXXX XXXX XXXX XXXX" className="w-full bg-black border border-white/10 p-5 rounded-xl text-sm font-bold text-amber-500 outline-none" value={formData.cardNumber.replace(/\s?/g, '').replace(/(\d{4})/g, '$1 ').trim()} onChange={e => setFormData({...formData, cardNumber: e.target.value.replace(/\s/g, '')})} />
                  <div className="grid grid-cols-2 gap-4">
                    <input required placeholder="MM / YY" maxLength="5" className="w-full bg-black border border-white/10 p-5 rounded-xl text-sm font-bold text-amber-500 outline-none" value={formData.expiry} onChange={e => { let val = e.target.value.replace(/\D/g, ''); if (val.length > 2) val = val.substring(0,2) + '/' + val.substring(2,4); setFormData({...formData, expiry: val}); }} />
                    <input required maxLength="3" placeholder="CVV" className="w-full bg-black border border-white/10 p-5 rounded-xl text-sm font-bold text-amber-500 outline-none" value={formData.cvv} onChange={e => setFormData({...formData, cvv: e.target.value.replace(/\D/g, '')})} />
                  </div>
                </motion.div>
              )}
